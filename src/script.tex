...
\title{Verkleinerung von probabilistischen Modellen für die Verwendung mit prism}
...
In vielen die Sicherheit von komplexen Systemen betreffenden Problemstellungen spielen probabilistische Modelle, insbesondere \mcpl mit diskreter Zeitauffassung, eine wichtige Rolle. Im konkreten Beispiel kann die Modellierung einer Flugzeugsteuerung und die Analyse resultierender Modelle auf gewisse Eigenschaften dazu beitragen, Abstürze von Flugzeugen zu verhindern.
Der PRSIM Model checker ist ein verbreitetes Tool, um jene Modelle auf gewissen Eingenschaften zu untersuchen. Dabei ist die Größe des Modells ein limitierender Faktor, da mit Modellen zunehmender Größe der Speicher- und Rechenaufwand von PRISM steigt. Ziel soll es daher in diesem Forschungsprojekt sein, Möglichkeiten zu untersuchen, wie jene Modelle in solche äquivalente kleinere Modelle umgewandelt werden können, die invariant bzgl. der zu analysierenden bzw. verifizierenden Eigenschaft zueinander und zum ursprünglichen Modell sind. Methoden zur Reduktion von Modellen zu finden, ist bis zu unserem Wissensstand ein noch recht offenes Forschungsfeld.
Verkleinern wollen wir in dieser Arbeit Modelle durch eine Reduktion der Anzahl der Variablen mittels Zusammenfassung redundanter Variablen. Mittels der Zahl der nodes und states, welche PRISM bei der Verarbeitung des verkleinerten Modells benötigt, werden wir beurteilen, wie erfolgsbringend eine solche Reduktion ist. \textit{In der Forschungsarbeit (referenz einfügen)...} wurde bereits ein Machbarkeitsbeweis geführt, dass tatsächlich eine Reduktion motiviert durch die Methode der Live Range Analysis aus dem Compilerbau möglich ist und diese ein erhebliche Verkleinerung von Modellen bewirken kann. In diesem Forschungsprojekt soll es schlussendlich darum gehen zu ausgewählten Modellen all jene minimalen reduzierten Modelle zu finden, welche mit kleinster Anzahl an Variablen auskommen. Wir wollen empirisch an beispielhaften Modellen zeigen, dass die Zahl der nodes und states bei der anschließenden Verarbeitung mit PRISM unter allen erhaltenen minimalen reduzierten Modellen variiert sowie Heuristiken vorstellen, welche jeweils zu einem konreteten reduzierten Modell führen und diese vergleichen.


\section{Grundlagen}


\subsection{Erkenntnisse aus bisheriger Forschung}

...hier detailliertere Erkenntnisse aus vorangegangener Forschungsarbeit.

\subsection{Methoden der Modellverkleinerung}

Zur Verkleinerung unserer Modelle adaptieren wir die Methode Live Range Analysis, die hauptsächlich aus dem Compilerbau bekannt ist. Zunächst soll diese Methode hier kurz beschrieben werden.

\subsubsection{Live Range Analysis}

Wenn ein Compiler in der Phase der Codegenerierung für die finale Plattform lokale Variablen aus Funktionen den Registern einer CPU zuweisen muss, so ist es für die Performance zur Laufzeit der entstehenden Anwendung sehr bedeutend, wie geschickt stets diejenigen Variablen in Registern gehalten werden, die als nächstes gelesen werden müssen. Zugriffe auf Cache oder Arbeitsspeicher dagegen gehen mit erheblichen Wartezeiten einher. Der auf der Hand liegende erste Schritt zur Steigerung der Laufzeitperformance ist es daher, keine Register mit solchen Variablen, die gar nicht mehr gebraucht werden, belegt und weiterhin reserviert zu haben, während dort schon neue Variablen gespeichert werden können.

Ein Programmablauf kann als gerichteter Graph $G = (V,E) $ aufgefasst werden, jede einzelne Position zwischen zwei Prozedurschritten ist ein Knoten $v\in V \neq \emptyset$, jeder Sprung und jeder simple Übergang von einer Position $v_1\in V$ über einen Prozedurschritt zu einer nächsten Position $v_2 \in V$ wird als Kante $(v_1, v_2) \in E \subseteq (V \times V)$ zwischen den Knoten definiert.
Dann sagen wir: 
\begin{definition}[Liveness]
	Eine Variable \textit{x} heißt \textit{live} in einem Knoten \textit{v}, wenn es von $v$ einen Pfad gibt, auf welchem $x$ gelesen wird, ohne dass zuvor auf diesem Pfad $x$ geschrieben wird. Wir schreiben dann $\textrm{live}_x(v)$ und sonst $\neg \textrm{live}_x(v)$.
\end{definition}

Man könnte auch sagen, ist eine Variable live, so wird sie eventuell noch gebraucht. Für den Fall, dass eine Variable auf allen Pfaden jeweils entweder nicht mehr gelesen wird, oder vor jedem Lesen geschrieben wird, so wird offensichtlich das Zwischenergebnis, welches in der Variablen abgelegt ist, nicht mehr gebraucht. Per Definition ist diese Variable dann nicht live, wir nennen sie dann auch \textit{dead}. In so einem Fall ließe sich ein Register wiederverwenden ab dem Zeitpunkt, zu dem die dort abgelegte Variable dead wird, man kann auch sagen, wenn sie ihre \textit{live range} verlässt.

\begin{definition}[Live Range]
	Zu einem Programmgraph $G = (V,E)$ bezeichnen wir mit $\textrm{live\_range}(x)$ für eine Variable $x$ die Menge aller Knoten $v\in V$, sodass $x$ live ist in $v$:
\[
	\textrm{live\_range}(x) \coloneqq \{v\in V \mid \textrm{live}_x(v) \}
\]
\end{definition}

Sinn und Zweck der Live Range Analysis ist es nun, durch Berechnung von $\textrm{live\_range}(x)$ für jede Variable $x$ festzustellen, dass sich Variablen $x_1, x_2, x_3 \dots$ mit paarweise disjunkten Mengen $\textrm{live\_range}(x_1), \textrm{live\_range}(x_2), \textrm{live\_range}(x_3), \dots$ ein und dasselbe Register teilen können.

Die bekannte iterative Methode zur Berechnung der Mengen $\textrm{LIVE}_v \coloneqq \{ x \in Var \mid \textrm{live}_x(v) \}$ wollen wir hier kurz wiedergeben:

Jeder Prozedurschritt kann einerseits Variablen lesen, womit diese unmittelbar vor diesem Schritt live werden. Andererseits können Variablen geschrieben werden. Dadurch werden diese an der Programmposition zuvor dead. Dieses Generieren bzw. Aufheben von Liveness drücken wir mithilfe von Mengen $\mathrm{GEN}_{(v_1,v_2)}$ und $\mathrm{KILL}_{(v_1,v_2)}$ aus:

\begin{align}
\mathrm{GEN}_{(v_1,v_2)} & \coloneqq \{x \in \mathrm{Var} \mid \text{$x$ gelesen, nicht zuvor $x$ geschrieben im Schritt $(v_1,v_2)$} \} \nonumber \\
\mathrm{KILL}_{(v_1,v_2)} & \coloneqq \{x \in \mathrm{Var} \mid \text{$x$ geschrieben , nicht zuvor $x$ gelesen im Schritt $(v_1,v_2)$} \} \nonumber
\end{align}

Bezeichnen wir weiterhin mit $\mathrm{LIVE}^{in}_{(v_1,v_2)}$ die Menge der Variablen, welche live sind unmittelbar vor Schritt $(v_1,v_2)$ und mit $\mathrm{LIVE}^{out}_{(v_1,v_2)}$ die Menge der Variablen, welche live sind unmittelbar nach Schritt $(v_1,v_2)$ im Programmfluss. Dann können wir $\mathrm{LIVE}^{in}_{(v_1,v_2)}$ mithilfe von $\mathrm{GEN}_{(v_1,v_2)}$ und $\mathrm{KILL}_{(v_1,v_2)}$ aus der Menge der live Variablen nach Schritt $(v_1,v_2)$ ausdrücken:

\begin{equation*}
\mathrm{LIVE}^{in}_{(v_1,v_2)} = \mathrm{GEN}_{(v_1,v_2)} \cup \big( \mathrm{LIVE}^{out}_{(v_1,v_2)} \setminus \mathrm{KILL}_{(v_1,v_2)} \big) 
\end{equation*}

An einer Position werden verschiedene Möglichkeiten des weiteren Programmverlaufs entsprechend akkumuliert:

\begin{equation*}
\mathrm{LIVE}^{out}_{(v_1,v_2)} = \bigcup_{v_3 \in V \textrm{ mit } (v_2,v_3)\in E} {\mathrm{LIVE}^{in}_{(v_2,v_3)}  }
\end{equation*}

Für den Fall eines Terminalzustandes $v_2$ ergibt sich die Menge $\mathrm{LIVE}^{out}_{(v_1,v_2)} = \emptyset$ entsprechend zur leeren Menge. Es ist aus der Literatur wohlbekannt, dass bei endlichen Graphen $G$ es genügt mit $\mathrm{LIVE}^{out}_{(v_1,v_2)} \coloneqq \emptyset$ und $\mathrm{LIVE}^{in}_{(v_1,v_2)} \coloneqq \emptyset$ für alle Kanten $(v_1,v_2)$ zu starten und schrittweise die Mengen anhand der Gleichungen zu aktualisieren bis keine Änderungen mehr auftreten. Das Verfahren terminiert nach endlichen Schritten und wir erhalten zu jeder Programmposition, d.h. zu jedem Knoten die Menge der Variablen, die dort live sind. Das Bestimmen von $\textrm{live\_range}(x)$ für eine Variable $x$ anschließend ist trivial. Eine valide Formulierung wäre genauso zu sagen, wir können zwei verschiedene Variablen $x, y$ stets dann zu einem Register (bzw. zu einer Variablen) zusammenfassen, wenn $\textrm{live\_range}(x) \cap \textrm{live\_range}(y) = \emptyset $.

\subsubsection{PRISMs Dateiformat}

Wir betrachten für unsere praktische Implementierung gewählte Beispiele, die als \mcpl{} in dem Dateiformat, wie es PRISM verwendet, vorliegen. Da unsere Hauptmotivation darin besteht, kleine Modelle zu erreichen, um diese dann mit Tools wie PRISM verarbeiten zu können, wollen wir einmal von einem Beispielmodell im entsprechenden Format ausgehen, davon ausgehend abstrahieren und unsere Annahmen festhalten.
...
Wir gehen weiterhin von genau einem sogenanntem Modul aus, welches aus einzelnen Transitionen besteht, die rein der Lesbarkeit wegen Namen wie \textit{goto\_1} oder \textit{reset1} tragen können.
...
Mit der Zeile textttt{formula failure = e = 0} wird eine Eigenschaft beschrieben, auf die wir das Modell gern hin untersuchen möchten mittels Tools wie PRISM.
...
Die Reduktion, welche wir betrachten wollen absiert auf dem Zusammenführen von Variablen mit disjunkten live ranges. Um die Invarianz bezüglich der durch PRISM zu untersuchenden Eigenschaft herzustellen, ermöglichen wir es, ausgewählte Variablen von der Zusammenführung auszuschließen. Wir wollen also die folgenden Annahmen für unsere Reduktion treffen:

\begin{enumerate}[(a)]
	\item Es gibt eine Variable, die als Kontrollflussvariable dient, wir nennen diese hier $cf$ (engl. für \textit{control flow}).
	\item In jeder Transition ist $cf$ in der Eintrittsbedingung festgelegt. (In allen Folgezustandsbedingungen steht $cf$ damit genauso fest, da $cf$ entweder explizit eien Wert zugewiesen bekommt oder unverändert bleibt.)
	\item Es gibt eine Liste von Variablen, die von der Reduktion auszuschließen sind.
\end{enumerate}

\subsubsection{Reduktion mittels live range analysis}
...
\begin{itemize}
	...
	\item endlich viele Transitionen $t_1, t_2, \dots , t_m$ als Paare von Eintrittsbedingung und Tupel der Folgezustandsbedingungen $t_i = (f^{enter}, (f^{leave}_1, f^{leave}_2, ... f^{leave}_{k_i}))$.
\end{itemize}

In der vorangegangenen Forschungsarbeit wurden alle Folgezustandsbedingungen einer Transition zu einer Transition zusammengefasst.
Die erste, jedoch natürlich offensichtliche Optimierung, die wir vornehmen wollen, ist es, jede Transition aus dem Dateiformat mit mehr als einer Folgezustandsbedingung anhand ebendieser in eigenständige Transitionen aufzusplitten. Dies können wir ohne Weiteres tun, da für unsere auf rein syntaktischer Basis funktionierende Reduktion den Wahrscheinlichkeiten keine Bedeutung zukommt.
Aus 
\begin{equation*}
	t_i = (f^{enter}, (f^{leave}_1, f^{leave}_2, ... f^{leave}_{k_i}))
\end{equation*}

werden somit die eigenständigen Transitionen

\begin{align*}
t_i^1 & = (f^{enter}, (f^{leave}_1)) \\
t_i^2 & = (f^{enter}, (f^{leave}_2)) \\
& \dots \\
t_i^{k_i} &= (f^{enter}, (f^{leave}_{k_i}))
\end{align*}

Identifizieren wir also jede resultierende Transition als ein $s_j = (f_j^{enter}, f_j^{leave})$, $j \in 1, \dots, J$ mit der Anzahl resultierender Transitionen $J \in \mathbb{N}$.
Wir erhalten unmittelbar einen Programmgraphen $G = (V,E)$, wie wir ihn als Ausgangspunkt für die live range analysis benötigen: 

\begin{itemize}
	\item $V\coloneqq D_{cf}$
	\item $E\coloneqq \big\{(v_1,v_2) \mid \exists j\in \{1, \dots , J\} \colon \text{\dq}cf = v_1 \in f_j^{enter}\text{\dq und \dq}cf = v_2 \in f_j^{leave}\text{\dq} \big\} $
\end{itemize}

Es kann dabei zu Mehrfachkanten kommen mit jeweils unterschiedlichen Transitionen $s_j$ {\large (TODO... besser formulieren und notieren)}
...
Dazu bilden wir den ungerichteten Graphen $H=(Var, F)$. Dabei sind die Knoten lediglich die Variablen.
...
\[
	\{x,y\} \in F \Leftrightarrow \exists v \in D_{cf} : \{x, y\} \subseteq \mbox{live\_vars}(v) 
\]
...
Um zu erreichen, dass ausgewählte Variablen von der Reduktion nicht betroffen sind, um gewünschte Eigenschaften der Modelle zu erhalten, muss eine solche Variable als Knoten des Graph $H$ lediglich mit allen anderen Variablen über eine Kante verbunden werden.

\section{Effiziente Methoden zur Berechnung aller Minimalen Colorings}

Wir wollen hier eine effiziente Methode vorstellen, die Menge aller größtmöglichen Reduktionen, d.h. die Menge aller k-Färbungen des Graphen $H$ mit kleinstmöglichem $k$ zu berechnen.

Dazu nutzen wir Paare von bitsets der Länge $|Var|$:

\begin{definition}[Variablenkombination]
	Wir nennen $k$ eine Variablenkombination, wenn es die Form $k = (id, neighbours)$ hat mit $id, neighbours : \mbox{Var} \to \{0,1\}$ und $id \cap neighbours = \emptyset$. Wir nennen $|id|$ die Größe der Variablenkombination.
\end{definition}

Eine Variablenkombination dient dazu sowohl ursprüngliche Variablen als auch durch Zusammenführen von Variablen entstehende neue Variablen darzustellen. $id$ und $neighbours$ stellen wir bei der Implementierung der Effizienz wegen als bitsets dar.
Jeder ursprünglichen Variablen $x \in Var$ ordnen wir die Variablenkombination $k_x \coloneqq (\{x\}, \{y \in Var \mid \{x,y\} \in F\})$ zu. Dabei beschreibt $id$, welche Variablen in der Kombination zusammengeführt worden sind und $neighbours$ beschreibt alle inzidenten anderen Variablen im Graph H, also genau alldiejenigen Variablen, mit welchen die Kombination nicht erweitert werden darf.

\begin{definition}[zusammenführbar]
	Wir nennen zwei Variablenkombinationen $k, l$ zusammenführbar, in Symbolen $k \downarrow l$, wenn $k_0 \cap l_1 \land l_0 \cap k_1$. Mit $k \cup l$ bezeichnen wir in diesem Fall die aus $k$ und $l$ nach Zusammenführen entstandene Variablenkombination  $k \cup l \coloneqq (k_0 \cup l_0, k_1 \cup l_1)$
\end{definition}

Die Definition zur Zusammenführbarkeit erlaubt die Zusammenführung offensichtlich genau in dem Fall, wenn ein keine zwei Variablen $x \in k_0$ und $y \in l_0$ existieren, welche eine Kante in $H$ verbindet.

Wir gehen nun algorithmisch wiefolgt vor:
...
Wir berechnen nun nacheinander $K_2, K_3, K_4, \dots $ durch Zusammenführen bereits bekannter Variablenkombinationen: Haben wir zuletzt $K_i$ berechnet, so berechnen wir $K_{i+1}$ wiefolgt:

\begin{lstlisting}[escapeinside={(*}{*)}]
(*$K_{i+1} := \emptyset$*)
for(b in (*$K_1$*)):
   for (k in (*$K_i$*)):
      if ((*$b \downarrow l$*) and (*$b_0 \cap l_0 = \emptyset$*)):
         (*$K_{i+1} := K_{i+1} \cup (b \cup l)$*)
\end{lstlisting}

Nach endlich vielen Schritten wird ein $K_i = \emptyset$ erreicht, sobald sich aus den Variablenkombinationen $K_{i-1}$ keine größeren mehr ableiten lassen. Alle Elemente von $K_{i-1}$ sind dann maximal:

\begin{definition}[maximale Variablenkombination]
	Eine Variablenkombination $k$ nennen wir maximal, wenn es keine Variablenkombination $l\in K_1$ gibt, sodass $k, l$ zusammenführbar sind.
\end{definition}
...
Diagramme zur Verteilung 


Heuristiken einzeichnen:
